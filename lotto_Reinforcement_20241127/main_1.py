...
class FileManager:
    ...
    def get_log_file(self) -> Path:
        ...
        return self.logs_dir / f'lotto_prediction_{current_date}.log'
...
class LogManager:
    ...
    def setup_logging(self):
        """로깅 설정"""
        log_file = self.get_new_log_file()
    ...
    def get_new_log_file(self) -> Path:
        ...
        # 해당 날짜의 기존 로그 파일들 확인
        existing_logs = list(log_dir.glob(f'lotto_prediction_{current_date}-*.log'))
        ...
        return log_dir / f'lotto_prediction_{current_date}-{new_number}.log'
    ...
    def log_debug(self, message: str):
        # DEBUG 레벨 로깅 제한
        if message.startswith("Selected numbers"):
            return
...
class LottoAnalyzer:
    ...
    def select_numbers(self) -> list:
        ...
        selected.sort()
        self.log_manager.log_debug(f"Selected numbers: {selected}")
        return selected
...
class LottoPredictionGUI:
    ...
    #
    #                 # 가중치 업데이트
    #                 predicted = self.analyzer.select_numbers()
    #                 matches = set(predicted) & set(actual_numbers)
    #                 if matches:
    ...
    #             if best_state:
    #                 summary_text = (
    #                     f"\n{draw_number}회차 학습 결과 ({iterations}회 시도):\n"
    #                     f"최고 점수: {best_score:.2f}\n"
    #                     f"매칭 분포:\n"
    #                 )
    #                 for matches, count in match_counts.items():
    #                     summary_text += f"{matches}개 일치: {count}회 ({count / iterations * 100:.2f}%)\n"
    #
    ...
    #             self.results_text.insert(tk.END, f"최고 성능 모델 점수: {overall_best_score:.2f}\n\n")
    #
    #             final_predictions = []
    ...
    def _save_model(self, model_state):
        ...
        try:
            current_datetime = datetime.now().strftime('%Y%m%d_%H%M%S')
            model_path = self.file_manager.models_dir / f'best_model_{current_datetime}.pkl'
    ...
    def _save_learning_results(self, learning_analyzer):
        ...
        try:
            current_datetime = datetime.now().strftime('%Y%m%d_%H%M%S')
            excel_path = self.file_manager.predictions_dir / f'learning_results_{current_datetime}.xlsx'
